...
const orders = new Array(ORDERS_COUNT).fill("_").map(() => {
    ...
    return {
        // Add other fields which are connected to other models later
        status: AvailableOrderStatuses[
            getRandomNumber(AvailableOrderStatuses.length)
        ],
        paymentProvider:
            paymentProvider === "UNKNOWN" ? "PAYPAL" : paymentProvider, // Avoid setting UNKNOWN payment provider
...
const seedEcomOrders = async () => {
    ...
    await EcomOrder.insertMany(
        orders.map((order) => {
            ...
            const orderInstance =
                orderPayload[getRandomNumber(orderPayload.length)]; // pick any random order instance to include
            return {
                ...order,
                ...
                address:
                    ...
                    addresses.find(
                        (add) =>
                            add.owner?.toString() === customer?._id.toString()
    ...
    return res
        ...
        .json(
            new ApiResponse(
                201,
                {},
                "Database populated for ecommerce successfully"
            )
        );
});
